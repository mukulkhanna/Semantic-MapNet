...
def get_projections_indices(file):
    ...
    h5file = h5py.File(os.path.join(f'data/{NAME}/smnet_training_data_indices', file), 'r')
...
print(
    " Start collapsing projections indices (Projecting the pixel with highest height in case of collision.)"
)
...
for split in ['train', 'val', 'test']:
    ...
    with h5py.File(
        os.path.join(output_root, output_name + "_{}.h5".format(split)), "w"
    ) as f:
    ...
    json.dump(
        projection_indices_envs,
        open(os.path.join(output_root, output_name + "_{}.json".format(split)), "w"),
    )
