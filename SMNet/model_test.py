...
class SMNet(nn.Module):
    def __init__(self, cfg, device):
        ...
        if mem_update == "lstm":
            ...
            noise = 0.01
            self.rnn.weight_hh.data = -noise + 2 * noise * torch.rand_like(
                self.rnn.weight_hh
            )
            self.rnn.weight_ih.data = -noise + 2 * noise * torch.rand_like(
                self.rnn.weight_ih
            )
            self.rnn.bias_hh.data = torch.zeros_like(
                self.rnn.bias_hh
            )  # redundant with bias_ih
            self.rnn.bias_ih.data = -noise + 2 * noise * torch.rand_like(
                self.rnn.bias_ih
            )
    ...
    def encode(self, features, proj_wtm, mask_outliers, heights, map_height, map_width):
        ...
        if self.mem_update == "lstm":
            state = (
                torch.zeros(
                    (map_height * map_width, self.mem_feat_dim),
                    dtype=torch.float,
        ...
        observed_masks = torch.zeros(
            (map_height * map_width), dtype=torch.bool, device=self.device
        )
        height_map = torch.zeros(
            (map_height * map_width), dtype=torch.float, device=self.device
        )
        ...
        for t in tqdm(range(T)):
            ...
            flat_indices = (
                map_width * world_to_map[:, :, 1] + world_to_map[:, :, 0]
            ).long()
    ...
    def forward(
        self, features, proj_wtm, mask_outliers, heights, map_height, map_width
    ):

        memory, observed_masks, height_map = self.encode(
            features, proj_wtm, mask_outliers, heights, map_height, map_width
        )
...
class SemmapDecoder(nn.Module):
    def __init__(self, feat_dim, n_obj_classes):
        ...
        self.obj_layer = nn.Sequential(
            ...
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True),
            nn.Conv2d(48, n_obj_classes, kernel_size=1, stride=1, padding=0, bias=True),
        )
